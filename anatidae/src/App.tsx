...
import { ANATIDAE_THEME } from "./colors";
import anatidaeData from "./data/anatidae.json";
...
const fullData = anatidaeData as TaxonNode;
const DOMESTIC_DUCK_ID = "DOMESTIC_DUCK";
const DOMESTIC_GOOSE_ID = "DOMESTIC_GOOSE";

function collapseTree(node: TaxonNode, duckExpanded: boolean, gooseExpanded: boolean): TaxonNode {
  if (node.id === DOMESTIC_DUCK_ID && !duckExpanded) {
    const { children: _c, ...rest } = node;
    return rest as TaxonNode;
  }
  if (node.id === DOMESTIC_GOOSE_ID && !gooseExpanded) {
    const { children: _c, ...rest } = node;
    return rest as TaxonNode;
  }
  ...
  return { ...node, children: node.children.map(c => collapseTree(c, duckExpanded, gooseExpanded)) };
}
...
export default function App() {
  ...
  const [duckBreedsExpanded, setDuckBreedsExpanded] = useState(false);
  const [gooseBreedsExpanded, setGooseBreedsExpanded] = useState(false);
  ...
  const treeData = useMemo<TaxonNode>(() => {
    const base = collapseTree(fullData, duckBreedsExpanded, gooseBreedsExpanded);
    if (!expandedSubspeciesId) return base;
    return expandOneSubspecies(base, fullData, expandedSubspeciesId);
  }, [duckBreedsExpanded, gooseBreedsExpanded, expandedSubspeciesId]);
  ...
  const handleSelect = useCallback((node: TaxonNode | null) => {
    setSelected(prev => {
      ...
      if (node?.rank === "SPECIES" &&
          node.id !== DOMESTIC_DUCK_ID &&
          node.id !== DOMESTIC_GOOSE_ID) {
      ...
      if (node?.id === DOMESTIC_DUCK_ID) {
        if (!duckBreedsExpanded) {
          pendingZoomId.current = DOMESTIC_DUCK_ID;
          setDuckBreedsExpanded(true);
          return node;
        } else if (isSame) {
          setDuckBreedsExpanded(false);
          return null;
        }
      }

      if (node?.id === DOMESTIC_GOOSE_ID) {
        if (!gooseBreedsExpanded) {
          pendingZoomId.current = DOMESTIC_GOOSE_ID;
          setGooseBreedsExpanded(true);
          return node;
        } else if (isSame) {
          setGooseBreedsExpanded(false);
          return null;
        }
      }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
          <span style={{ fontSize: 18, fontWeight: 600, letterSpacing: "-0.01em" }}>Anatidae</span>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          {duckBreedsExpanded && (
            <button onClick={() => { setDuckBreedsExpanded(false); setSelected(null); }}
              style={{ ...btnBase, borderColor: "#1a3028", background: "#0f1a18", color: ANATIDAE_THEME.breedGroupColor, marginRight: 4 }}>
              ← Hide duck breeds
            </button>
          )}
          {gooseBreedsExpanded && (
            <button onClick={() => { setGooseBreedsExpanded(false); setSelected(null); }}
              style={{ ...btnBase, borderColor: "#1a3028", background: "#0f1a18", color: ANATIDAE_THEME.breedGroupColor, marginRight: 4 }}>
              ← Hide goose breeds
            </button>
          )}
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        <div style={{ flex: 1, overflow: "hidden" }}>
          <FamilyTree
            ...
            highlightedNodeIds={highlightedNodeIds} colorTheme={ANATIDAE_THEME}
            specialNodeId={[DOMESTIC_DUCK_ID, DOMESTIC_GOOSE_ID]}
          />
        </div>
