...
function LinkRow({ href, label }: { href: string; label: string }) {
  return (
    <a href={href} target="_blank" rel="noopener noreferrer" style={{
      ...
      color: "#8899bb", textDecoration: "none", fontSize: 12,
    }}>
      <span>{label}</span>
  ...
  return (
    <li onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{ cursor: "pointer", padding: "4px 6px", borderRadius: 4, marginBottom: 2,
        background: hover ? "rgba(255,255,255,0.05)" : "transparent", listStyle: "none" }}>
      {children}
    </li>
  );
}
  ...
  return (
    <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
      ...
      {node.subspeciesCount !== undefined && node.subspeciesCount > 0 && subspecies.length === 0 && (
        <div style={{ fontSize: 13, color: "#666", marginBottom: 4 }}>{node.subspeciesCount} subspecies · click to expand</div>
      )}
      {node.children?.some(c => c.rank === "BREED_GROUP") && (
        <div style={{ fontSize: 13, color: BREED_GROUP_COLOR, marginBottom: 4 }}>
          {node.children.reduce((s, c) => s + countLeaves(c), 0)} recognised breeds · click to expand
        </div>
      )}
...
function SubspeciesPanel({ node }: { node: TaxonNode; onSelect: (n: TaxonNode) => void; findNodeById: (id: string) => TaxonNode | null }) {
  ...
  return (
    <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
      ...
      <div style={{ fontSize: 20, fontWeight: 600, color: node.accepted === false ? "#666" : accent, fontStyle: "italic", marginBottom: 2 }}>{node.name}</div>
...
function BreedPanel({ node }: { node: TaxonNode; onSelect: (n: TaxonNode) => void; findNodeById: (id: string) => TaxonNode | null }) {
  ...
  const extract = wiki?.extract ?? null;
  const wikiUrl = wiki?.content_urls?.desktop?.page;
  const isDuck = node.lineage === "Dabbling Duck";
  return (
    <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
      <div style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", marginBottom: 10 }}>
        Domestic breed · {isDuck ? "Duck" : "Goose"}
      </div>
      <div style={{ fontSize: 22, fontWeight: 600, color: BREED_GROUP_COLOR, marginBottom: 2 }}>{node.name}</div>
      {node.origin && <div style={{ fontSize: 16, color: "#aaa", marginBottom: 8 }}>{node.origin}</div>}
      {loading && <div style={{ marginTop: 16, width: "100%", height: 120, background: "#1a1a2a", borderRadius: 6 }} />}
      {!loading && wiki?.thumbnail?.source && (
      ...
      <div style={{ marginTop: 16 }}>
        ...
        {wikiUrl && <LinkRow href={wikiUrl} label="Wikipedia" />}
        <LinkRow
          href={isDuck ? "https://en.wikipedia.org/wiki/Domestic_duck" : "https://en.wikipedia.org/wiki/Domestic_goose"}
          label={isDuck ? "Domestic Duck (Wikipedia)" : "Domestic Goose (Wikipedia)"}
        />
      </div>
    </div>
  );
...
function HomePanel({ onSelect: _onSelect }: { subfamilies: TaxonNode[]; onSelect: (n: TaxonNode) => void }) {
  return (
    <div style={{ padding: "24px 20px", color: "#888", fontSize: 13, lineHeight: 1.6 }}>
      ...
      <p>Click any node to explore. Click a species with the dashed ring to expand subspecies. Click Domestic Duck or Domestic Goose to reveal breeds.</p>
      ...
      <div style={{ marginTop: 20, padding: "12px 14px", borderRadius: 8, background: "#111", fontSize: 12, lineHeight: 1.7 }}>
        ...
        <div style={{ color: "#666" }}>
          Anatidae is the family of birds commonly known as ducks, geese, and swans. Found on
          every continent except Antarctica, they are adapted for life on water with webbed feet
          and waterproof plumage. The domestic duck descends mainly from the Mallard
          (<em style={{ fontStyle: "italic" }}>Anas platyrhynchos</em>), while domestic geese
          were bred from the Greylag Goose — and in Asia, the Swan Goose.
        </div>
      </div>
      <div style={{ marginTop: 28, fontSize: 11, color: "#444" }}>
        Data: Catalogue of Life ChecklistBank · Breeds: breed registries
      </div>
    </div>
  );
}
...
export default function InfoPanel({ node, onSelect, findNodeById, subfamilies = [] }: Props) {
  if (!node || node.rank === "FAMILY") return <HomePanel subfamilies={subfamilies} onSelect={onSelect} />;
    ...
    return (
      <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
        ...
        <div style={{ marginTop: 20 }}>
          <div style={{ fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "#555", marginBottom: 8 }}>Breeds</div>
          <ul style={{ padding: 0, margin: 0 }}>
            {breeds.map(b => (
  ...
  return (
    <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
      ...
      <div style={{ fontSize: 14, color: "#aaa", marginTop: 8 }}>
        {speciesLeaves.length} species{speciesLeaves.length < leafCount ? ` · ${leafCount - speciesLeaves.length} breeds` : ""}
      </div>
