...
function accentFor(node: TaxonNode): string {
  if (node.rank === "SUBFAMILY" || node.rank === "FAMILY") return "#F5F5F5";
  ...
  return (
    <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        {wikiUrl && <LinkRow href={wikiUrl} label="Wikipedia" />}
        <LinkRow
          href="https://en.wikipedia.org/wiki/Domestic_pig"
          label="Domestic Pig (Wikipedia)"
        />
      </div>
    </div>
  );
...
function HomePanel({ onSelect: _onSelect }: { subfamilies: TaxonNode[]; onSelect: (n: TaxonNode) => void }) {
  return (
    <div style={{ padding: "24px 20px", color: "#888", fontSize: 13, lineHeight: 1.6 }}>
      ...
      <p>Click any node to explore. Click a species with the dashed ring to expand subspecies. Click Domestic Pig to reveal breeds.</p>
      ...
      <div style={{ marginTop: 20, padding: "12px 14px", borderRadius: 8, background: "#111", fontSize: 12, lineHeight: 1.7 }}>
        ...
        <div style={{ color: "#666" }}>
          Suidae is the family of even-toed ungulates commonly known as pigs. It includes
          the wild boar and its many relatives across Africa and Asia — from the tusked
          babirusa of Sulawesi to the warthog of the African savanna. The domestic pig
          descends from the wild boar (<em style={{ fontStyle: "italic" }}>Sus scrofa</em>)
          and was independently domesticated in multiple regions around 9,000 years ago.
        </div>
      </div>
...
export default function InfoPanel({ node, onSelect, findNodeById, subfamilies = [] }: Props) {
  if (!node || node.rank === "SUBFAMILY" || node.rank === "FAMILY") {
