...
import { SUIDAE_THEME } from "./colors";
import suidaeData from "./data/suidae.json";
...
const fullData = suidaeData as TaxonNode;
const DOMESTIC_PIG_ID = "DOMESTIC_PIG";

function collapseTree(node: TaxonNode, pigExpanded: boolean): TaxonNode {
  if (node.id === DOMESTIC_PIG_ID && !pigExpanded) {
  ...
  return { ...node, children: node.children.map(c => collapseTree(c, pigExpanded)) };
}
...
export default function App() {
  ...
  const [pigBreedsExpanded, setPigBreedsExpanded] = useState(false);
  ...
  const handleSelect = useCallback((node: TaxonNode | null) => {
    setSelected(prev => {
      ...
      if (node?.id === DOMESTIC_PIG_ID) {
        if (!pigBreedsExpanded) {
          pendingZoomId.current = DOMESTIC_PIG_ID;
          setPigBreedsExpanded(true);
          return node;
        } else if (isSame) {
          setPigBreedsExpanded(false);
          return null;
        }
      }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
          <span style={{ fontSize: 18, fontWeight: 600, letterSpacing: "-0.01em" }}>Suidae</span>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          {pigBreedsExpanded && (
            <button
              onClick={() => { setPigBreedsExpanded(false); setSelected(null); }}
              style={{ ...btnBase, borderColor: "#3a3020", background: "#1a1510", color: SUIDAE_THEME.breedGroupColor, marginRight: 4 }}
            >
              ← Hide pig breeds
            </button>
          )}
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        <div style={{ flex: 1, overflow: "hidden" }}>
          <FamilyTree
            ...
            colorTheme={SUIDAE_THEME}
            specialNodeId={[DOMESTIC_PIG_ID]}
          />
        </div>
        <div style={{
          width: 380,
