  ...
  return (
    <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        {wikiUrl && <LinkRow href={wikiUrl} label="Wikipedia" />}
        <LinkRow
          href={isChicken ? "https://en.wikipedia.org/wiki/Chicken" : "https://en.wikipedia.org/wiki/Domestic_turkey"}
          label={isChicken ? "Chicken (Wikipedia)" : "Domestic Turkey (Wikipedia)"}
        />
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: "24px 20px", color: "#888", fontSize: 13, lineHeight: 1.6 }}>
      <div style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", marginBottom: 16 }}>
        Phasianidae · Family Tree
      </div>
      <p>Click any node to explore. Click a species with the dashed ring to expand subspecies. Click Domestic Chicken or Domestic Turkey to reveal breeds.</p>
      ...
      <div style={{ marginTop: 20, padding: "12px 14px", borderRadius: 8, background: "#111", fontSize: 12, lineHeight: 1.7 }}>
        ...
        <div style={{ color: "#666" }}>
          Phasianidae is the largest family in the order Galliformes, encompassing pheasants,
          partridges, quails, grouse, turkeys, peacocks, and junglefowl. The domestic chicken
          descends from the Red Junglefowl (<em style={{ fontStyle: "italic" }}>Gallus gallus</em>)
          and was first domesticated in Southeast Asia around 8,000 years ago. Today it is
          the world's most numerous bird.
        </div>
      </div>
      <div style={{ marginTop: 28, fontSize: 11, color: "#444" }}>
        Data: Catalogue of Life ChecklistBank · Breeds: APA / breed registries
      </div>
    </div>
  );
}
...
export default function InfoPanel({ node, onSelect, findNodeById, subfamilies = [] }: Props) {
  ...
  if (node.rank === "BREED_GROUP") {
    ...
    return (
      <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
        <div style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", marginBottom: 10 }}>
          {isChicken ? "Chicken" : "Turkey"} breed group
        </div>
