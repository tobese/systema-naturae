...
import { PHASIANIDAE_THEME } from "./colors";
import phasianidaeData from "./data/phasianidae.json";
...
const fullData = phasianidaeData as TaxonNode;
const DOMESTIC_CHICKEN_ID = "DOMESTIC_CHICKEN";
const DOMESTIC_TURKEY_ID = "DOMESTIC_TURKEY";

function collapseTree(node: TaxonNode, chickenExpanded: boolean, turkeyExpanded: boolean): TaxonNode {
  if (node.id === DOMESTIC_CHICKEN_ID && !chickenExpanded) {
    const { children: _c, ...rest } = node;
    return rest as TaxonNode;
  }
  if (node.id === DOMESTIC_TURKEY_ID && !turkeyExpanded) {
    const { children: _c, ...rest } = node;
    return rest as TaxonNode;
  }
  ...
  return { ...node, children: node.children.map(c => collapseTree(c, chickenExpanded, turkeyExpanded)) };
}
...
export default function App() {
  ...
  const [chickenBreedsExpanded, setChickenBreedsExpanded] = useState(false);
  const [turkeyBreedsExpanded, setTurkeyBreedsExpanded] = useState(false);
  ...
  const treeData = useMemo<TaxonNode>(() => {
    const base = collapseTree(fullData, chickenBreedsExpanded, turkeyBreedsExpanded);
    if (!expandedSubspeciesId) return base;
    return expandOneSubspecies(base, fullData, expandedSubspeciesId);
  }, [chickenBreedsExpanded, turkeyBreedsExpanded, expandedSubspeciesId]);
  ...
  const handleSelect = useCallback((node: TaxonNode | null) => {
    setSelected(prev => {
      ...
      if (node?.rank === "SPECIES" &&
          node.id !== DOMESTIC_CHICKEN_ID &&
          node.id !== DOMESTIC_TURKEY_ID) {
      ...
      if (node?.id === DOMESTIC_CHICKEN_ID) {
        if (!chickenBreedsExpanded) {
          pendingZoomId.current = DOMESTIC_CHICKEN_ID;
          setChickenBreedsExpanded(true);
          return node;
        } else if (isSame) {
          setChickenBreedsExpanded(false);
          return null;
        }
      }

      if (node?.id === DOMESTIC_TURKEY_ID) {
        if (!turkeyBreedsExpanded) {
          pendingZoomId.current = DOMESTIC_TURKEY_ID;
          setTurkeyBreedsExpanded(true);
          return node;
        } else if (isSame) {
          setTurkeyBreedsExpanded(false);
          return null;
        }
      }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: "12px 20px", borderBottom: "1px solid #1e2030", flexShrink: 0,
      }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
          <span style={{ fontSize: 18, fontWeight: 600, letterSpacing: "-0.01em" }}>Phasianidae</span>
          <span style={{ fontSize: 13, color: "#555" }}>Family Tree · {speciesCount} species</span>
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {expandedSubspeciesId && (
            <button onClick={handleCollapseSubspecies}
              style={{ ...btnBase, borderColor: "#303048", background: "#141420", color: "#8899cc", marginRight: 4 }}>
              ← Collapse subspecies
            </button>
          )}
          {chickenBreedsExpanded && (
            <button onClick={() => { setChickenBreedsExpanded(false); setSelected(null); }}
              style={{ ...btnBase, borderColor: "#3a3020", background: "#1a1510", color: PHASIANIDAE_THEME.breedGroupColor, marginRight: 4 }}>
              ← Hide chicken breeds
            </button>
          )}
          {turkeyBreedsExpanded && (
            <button onClick={() => { setTurkeyBreedsExpanded(false); setSelected(null); }}
              style={{ ...btnBase, borderColor: "#3a3020", background: "#1a1510", color: PHASIANIDAE_THEME.breedGroupColor, marginRight: 4 }}>
              ← Hide turkey breeds
            </button>
          )}
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        <div style={{ flex: 1, overflow: "hidden" }}>
          <FamilyTree
            data={treeData} layout={layout} onSelect={handleSelect}
            selectedId={selected?.id ?? null} pendingZoomId={pendingZoomId}
            highlightedNodeIds={highlightedNodeIds} colorTheme={PHASIANIDAE_THEME}
            specialNodeId={[DOMESTIC_CHICKEN_ID, DOMESTIC_TURKEY_ID]}
          />
        </div>
        <div style={{ width: 380, borderLeft: "1px solid #1e2030", display: "flex", flexDirection: "column", flexShrink: 0 }}>
          ...
          />
          {selected && navContext && (
            <NodeNav parent={navContext.parent} siblings={navContext.siblings} index={navContext.index} onNavigate={handleSelect} />
          )}
          <div style={{ flex: 1, overflowY: "auto" }}>
            <InfoPanel node={selectedInTree} onSelect={handleSelect} findNodeById={(id) => walkFind(fullData, id)} subfamilies={[]} />
          </div>
        </div>
      </div>
    </div>
