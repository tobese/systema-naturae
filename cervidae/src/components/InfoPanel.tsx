...
function HomePanel({ subfamilies, onSelect }: { subfamilies: TaxonNode[]; onSelect: (n: TaxonNode) => void }) {
  return (
    <div style={{ padding: "24px 20px", color: "#888", fontSize: 13, lineHeight: 1.6 }}>
          ...
          {subfamilies.map(sf => (
            <button
              ...
              style={{
                ...
                color: SUBFAMILY_COLORS[sf.name] ?? "#8899bb", fontSize: 13,
              }}
            >
      ...
      <div style={{ marginTop: 24, padding: "12px 14px", borderRadius: 8, background: "#111", fontSize: 12, lineHeight: 1.7 }}>
      ...
      <div style={{ marginTop: 20, padding: "12px 14px", borderRadius: 8, background: "#111", fontSize: 12, lineHeight: 1.7 }}>
        ...
        <div style={{ color: "#666" }}>
          Cervidae is the deer family — hoofed ruminants found on every continent except
          Antarctica and Australia. Most males (and female reindeer) grow and shed antlers
          annually. The family spans from the smallest deer on Earth, the Southern pudú at
          ~8 kg, to the moose (Swedish: <em style={{ color: "#5878A8" }}>älg</em>), which
          can exceed 700 kg.
        </div>
      </div>

      <div style={{ marginTop: 20, fontSize: 11, color: "#444" }}>
        Data: Catalogue of Life ChecklistBank
      </div>
    </div>
  );
}
