...
export function useReadingWindow(orderedSlugs: string[], focusSlug?: string): ReadingWindow {
  // The window starts *at* the focused family rather than at the top, and
  // pins it, so a host app (the portal's Book viewMode) can open a chapter
  // already scrolled to the family the reader came from. Undefined in the
  // standalone, where a chapter always starts at its first family.
  const focusIndex = focusSlug ? orderedSlugs.indexOf(focusSlug) : -1;
  const [activeIndex, setActiveIndex] = useState(focusIndex > 0 ? focusIndex : 0);
  const [pinned, setPinned] = useState<Set<string>>(() => new Set(focusIndex > 0 ? [focusSlug!] : []));
