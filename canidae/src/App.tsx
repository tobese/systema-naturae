...
import { CANIDAE_THEME } from "./colors";
import canidaeData from "./data/canidae.json";
...
const fullData = canidaeData as TaxonNode;
const DOMESTIC_DOG_ID = "DOMESTIC_DOG";

function collapseTree(node: TaxonNode): TaxonNode {
  if (node.id === DOMESTIC_DOG_ID) {
...
export default function App() {
  ...
  const handleSelect = useCallback((node: TaxonNode | null) => {
    setSelected(prev => {
      ...
      if (node?.rank === "SPECIES" && node.id !== DOMESTIC_DOG_ID) {
      ...
      if (node?.id === DOMESTIC_DOG_ID) {
        if (!breedsExpanded) {
          pendingZoomId.current = DOMESTIC_DOG_ID;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
          <span style={{ fontSize: 18, fontWeight: 600, letterSpacing: "-0.01em" }}>Canidae</span>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          {breedsExpanded && (
            <button
              ...
              style={{ ...btnBase, borderColor: "#3a3050", background: "#1a1520", color: CANIDAE_THEME.breedGroupColor, marginRight: 4 }}
            >
              ← Full tree
            </button>
          )}
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        <div style={{ flex: 1, overflow: "hidden" }}>
          <FamilyTree
            ...
            colorTheme={CANIDAE_THEME}
            specialNodeId={DOMESTIC_DOG_ID}
          />
        </div>
        <div style={{
          width: 380,
