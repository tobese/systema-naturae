...
import { FELIDAE_THEME } from "./colors";
import felidaeData from "./data/felidae.json";
...
export default function App() {
  ...
  const treeData = useMemo<TaxonNode>(() => {
    const base = breedsExpanded
      ? expandOneSubspecies(collapsedData, fullData, FELIS_CATUS_ID)
      : collapsedData;
  ...
  const handleSelect = useCallback((node: TaxonNode | null) => {
    setSelected(prev => {
      ...
      if (node?.rank === "SPECIES" && node.id !== FELIS_CATUS_ID) {
      ...
      if (node?.id === FELIS_CATUS_ID) {
        if (!breedsExpanded) {
          pendingZoomId.current = FELIS_CATUS_ID;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
          <span style={{ fontSize: 18, fontWeight: 600, letterSpacing: "-0.01em" }}>Felidae</span>
          <span style={{ fontSize: 13, color: "#555" }}>Family Tree · 46 species</span>
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          {breedsExpanded && (
            <button
              ...
              style={{ ...btnBase, borderColor: "#3a3050", background: "#1a1520", color: "#A09CDC", marginRight: 4 }}
            >
              ← Full tree
            </button>
          )}
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        <div style={{ flex: 1, overflow: "hidden" }}>
          <FamilyTree
            ...
            colorTheme={FELIDAE_THEME}
            specialNodeId={FELIS_CATUS_ID}
          />
        </div>
        <div style={{
          width: 380,
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            <InfoPanel
              ...
              subfamilies={fullData.children?.filter(c => c.rank === "SUBFAMILY") ?? []}
            />
          </div>
        </div>
      </div>
