...
function accentFor(node: TaxonNode): string {
  if (node.rank === "TRIBE" || node.rank === "SUBFAMILY" || node.rank === "FAMILY") return "#F5F5F5";
...
function BreedPanel({ node }: { node: TaxonNode; onSelect: (n: TaxonNode) => void; findNodeById: (id: string) => TaxonNode | null }) {
  ...
  const extract = wiki?.extract ?? null;
  const wikiUrl = wiki?.content_urls?.desktop?.page;
  const isSheep = node.lineage === "Sheep";

  return (
    <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
      <div style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", marginBottom: 10 }}>
        Domestic breed · {isSheep ? "Sheep" : "Goat"}
      </div>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        {wikiUrl && <LinkRow href={wikiUrl} label="Wikipedia" />}
        <LinkRow
          href={isSheep ? "https://en.wikipedia.org/wiki/Domestic_sheep" : "https://en.wikipedia.org/wiki/Domestic_goat"}
          label={isSheep ? "Domestic Sheep (Wikipedia)" : "Domestic Goat (Wikipedia)"}
        />
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: "24px 20px", color: "#888", fontSize: 13, lineHeight: 1.6 }}>
      <div style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", marginBottom: 16 }}>
        Caprinae · Family Tree
      </div>
      <p>Click any node to explore. Click a species with the dashed ring to expand subspecies. Click Domestic Sheep or Domestic Goat to reveal breeds.</p>
      ...
      <div style={{ marginTop: 20, padding: "12px 14px", borderRadius: 8, background: "#111", fontSize: 12, lineHeight: 1.7 }}>
        ...
        <div style={{ color: "#666" }}>
          Caprinae is a subfamily of Bovidae encompassing sheep, goats, and their
          mountain-dwelling relatives. The group ranges from Arctic musk oxen to
          Himalayan blue sheep, united by adaptations for rugged highland terrain.
          Domesticated sheep and goats were among the first animals tamed by humans,
          around 10,000 years ago in the Near East.
        </div>
      </div>
...
export default function InfoPanel({ node, onSelect, findNodeById, subfamilies = [] }: Props) {
  if (!node || node.rank === "TRIBE" || node.rank === "SUBFAMILY" || node.rank === "FAMILY") {
  ...
  if (node.rank === "BREED_GROUP") {
    ...
    return (
      <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
        <div style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", marginBottom: 10 }}>
          {isSheep ? "Sheep" : "Goat"} breed group
        </div>
