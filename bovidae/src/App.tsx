...
import { BOVIDAE_THEME } from "./colors";
import bovidaeData from "./data/bovidae.json";
...
const fullData = bovidaeData as TaxonNode;
const BOS_TAURUS_ID = "BOS_TAURUS";
const BOS_INDICUS_ID = "BOS_INDICUS";
const BREED_EXPANDABLE = new Set([BOS_TAURUS_ID, BOS_INDICUS_ID]);

function collapseTree(node: TaxonNode): TaxonNode {
  if (BREED_EXPANDABLE.has(node.id)) {
...
function expandNode(base: TaxonNode, full: TaxonNode, targetId: string): TaxonNode {
  if (base.id === targetId) return walkFind(full, targetId) ?? base;
  if (!base.children) return base;
  return { ...base, children: base.children.map(c => expandNode(c, full, targetId)) };
}
...
export default function App() {
  ...
  const treeData = useMemo<TaxonNode>(() => {
    let base = collapsedData;
    for (const id of Array.from(expandedBreedIds)) {
      base = expandNode(base, fullData, id);
    }
    if (expandedSubspeciesId) {
      base = expandNode(base, fullData, expandedSubspeciesId);
    }
    return base;
  }, [expandedBreedIds, expandedSubspeciesId]);

  const handleSelect = useCallback((node: TaxonNode | null) => {
    setSelected(prev => {
      const isSame = prev?.id === node?.id;

      if (node?.rank === "SPECIES" && !BREED_EXPANDABLE.has(node.id)) {
      ...
      if (node && BREED_EXPANDABLE.has(node.id)) {
        if (!expandedBreedIds.has(node.id)) {
          pendingZoomId.current = node.id;
          setExpandedBreedIds(prev => { const s = new Set(prev); s.add(node.id); return s; });
          return node;
        } else if (isSame) {
          setExpandedBreedIds(prev => { const s = new Set(prev); s.delete(node.id); return s; });
          return null;
        }
      }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
          <span style={{ fontSize: 18, fontWeight: 600, letterSpacing: "-0.01em" }}>Bovidae</span>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          {expandedBreedIds.size > 0 && (
            <button
              onClick={handleCollapse}
              style={{ ...btnBase, borderColor: "#3a3020", background: "#1a1510", color: BOVIDAE_THEME.breedGroupColor, marginRight: 4 }}
            >
              ← Full tree
            </button>
          )}
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        <div style={{ flex: 1, overflow: "hidden" }}>
          <FamilyTree
            ...
            colorTheme={BOVIDAE_THEME}
            specialNodeId={[BOS_TAURUS_ID, BOS_INDICUS_ID]}
          />
        </div>
        <div style={{
          width: 380,
