...
import { CETACEA_THEME } from "./colors";
import cetaceaData from "./data/cetacea.json";
...
const fullData = cetaceaData as TaxonNode;
...
export default function App() {
  ...
  const treeData = useMemo<TaxonNode>(() => {
    if (!expandedSubspeciesId) return collapsedData;
    return expandOneSubspecies(collapsedData, fullData, expandedSubspeciesId);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
          <span style={{ fontSize: 18, fontWeight: 600, letterSpacing: "-0.01em" }}>Cetacea</span>
          <span style={{ fontSize: 13, color: "#555" }}>Whales, Dolphins & Porpoises · {speciesCount} species</span>
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {expandedSubspeciesId && (
            <button
              onClick={handleCollapseSubspecies}
              style={{ ...btnBase, borderColor: "#1a2a40", background: "#0e1a2a", color: "#6699cc", marginRight: 4 }}
            >
              ← Collapse subspecies
            </button>
          )}
          {(["radial", "vertical"] as const).map(l => (
            <button
              ...
              style={{
                ...btnBase,
                borderColor: layout === l ? "#1a2a40" : "#0e1a2a",
                background: layout === l ? "#0e1a2a" : "transparent",
