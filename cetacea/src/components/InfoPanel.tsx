...
import { LINEAGE_COLORS, SUBFAMILY_COLORS } from "../colors";
...
function SpeciesPanel({ node, onSelect }: { node: TaxonNode; onSelect: (n: TaxonNode) => void }) {
  ...
  return (
    <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
      ...
      {loading && (
        <div style={{ marginTop: 16, width: "100%", height: 120, background: "#0e1a2a", borderRadius: 6 }} />
      )}
...
function HomePanel({ subfamilies, onSelect }: { subfamilies: TaxonNode[]; onSelect: (n: TaxonNode) => void }) {
  return (
    <div style={{ padding: "24px 20px", color: "#888", fontSize: 13, lineHeight: 1.6 }}>
      ...
      <p>Click any species to explore. Click a species with the dashed ring to expand its subspecies.</p>
          ...
          {subfamilies.map(sf => (
            <button
              ...
              style={{
                ...
                background: "none", border: "none", borderBottom: "1px solid #0e1a2a",
                padding: "7px 0", cursor: "pointer",
                color: SUBFAMILY_COLORS[sf.name] ?? "#6699bb", fontSize: 13,
              }}
            >
      ...
      <div style={{ marginTop: 24, padding: "12px 14px", borderRadius: 8, background: "#080e18", fontSize: 12, lineHeight: 1.7 }}>
      ...
      <div style={{ marginTop: 20, padding: "12px 14px", borderRadius: 8, background: "#080e18", fontSize: 12, lineHeight: 1.7 }}>
        <div style={{ color: "#888", marginBottom: 4 }}>About</div>
        <div style={{ color: "#666" }}>
          Cetacea is the order of fully aquatic mammals comprising whales, dolphins, and
          porpoises — roughly 90 living species descended from land-dwelling artiodactyls
          related to hippos. Divided into <em style={{ color: "#1a6b8a" }}>Mysticeti</em> (baleen
          whales, who filter-feed through keratin plates) and{" "}
          <em style={{ color: "#0d4f6e" }}>Odontoceti</em> (toothed whales, who hunt with
          echolocation). The blue whale, at up to 30 m and 190 tonnes, is the largest animal
          known to have ever existed.
        </div>
      </div>

      <div style={{ marginTop: 20, fontSize: 11, color: "#444" }}>
        Data: Catalogue of Life ChecklistBank · Wilson & Reeder (2005)
      </div>
    </div>
  );
}
...
export default function InfoPanel({ node, onSelect, findNodeById, subfamilies = [] }: Props) {
  ...
  if (node.rank === "SUBFAMILY") {
    const accent = SUBFAMILY_COLORS[node.name] ?? "#888";
    ...
    return (
      <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
        ...
        <div style={{ fontSize: 13, color: "#666" }}>{speciesLeaves.length} species in this collection</div>
      </div>
    );
  }

  if (node.rank === "TRIBE") {
    const accent = node.lineage ? (LINEAGE_COLORS[node.lineage] ?? "#888") : "#888";
    const speciesLeaves = collectLeaves(node).filter(l => l.rank === "SPECIES");
    return (
      <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
        <div style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase", color: "#555", marginBottom: 10 }}>
  ...
  if (node.rank === "GENUS") {
    ...
    return (
      <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
        ...
        <div style={{ fontSize: 22, fontWeight: 600, color: accent, fontStyle: "italic", marginBottom: 4 }}>{node.name}</div>
        <div style={{ fontSize: 13, color: "#666", marginBottom: 16 }}>{speciesLeaves.length} {speciesLeaves.length === 1 ? "species" : "species"}</div>
