...
function SubspeciesPanel({ node, onSelect: _onSelect, findNodeById: _findNodeById }: { node: TaxonNode; onSelect: (n: TaxonNode) => void; findNodeById: (id: string) => TaxonNode | null }) {
  const accent = accentFor(node);
  const { data: wiki, loading } = useWikipediaSummary(node.name);
  const extract = wiki?.extract ?? null;
  const wikiUrl = wiki?.content_urls?.desktop?.page;
...
function BreedPanel({ node, onSelect, findNodeById }: { node: TaxonNode; onSelect: (n: TaxonNode) => void; findNodeById: (id: string) => TaxonNode | null }) {
  ...
  return (
    <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
      ...
      {node.origin && (
        <div style={{ fontSize: 16, color: "#aaa", marginBottom: 4 }}>{node.origin}</div>
      )}
      {node.coatType && (
        <div style={{ fontSize: 13, color: "#aaa", marginBottom: 8 }}>
          ...
          {node.coatType}
          {node.coatType === "rex" && (
            <span style={{ color: COAT_TYPE_COLOR, marginLeft: 8, fontSize: 12 }}>↔ connected in tree</span>
          )}
        </div>
      )}
      ...
      {node.wildParentId && (
        <div style={{ marginTop: 14 }}>
          <div style={{ fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "#444", marginBottom: 6 }}>
            Wild origin
          </div>
          <button
            ...
            onMouseEnter={e => { e.currentTarget.style.borderColor = BREED_GROUP_COLOR; e.currentTarget.style.color = "#fff"; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = "#1e2030"; e.currentTarget.style.color = "#bbb"; }}
          >
...
function HomePanel({ subfamilies, onSelect }: { subfamilies: TaxonNode[]; onSelect: (n: TaxonNode) => void }) {
  ...
  const sfMeta: Record<string, { species: string; extra?: string }> = {
    Pantherinae: { species: "2 genera · 7 species" },
    Felinae:     { species: "12 genera · ~39 species", extra: "7 lineages" },
  };
  ...
  return (
    <div style={{ padding: "24px 20px", color: "#888", fontSize: 13, lineHeight: 1.6 }}>
      ...
      {subfamilies.length > 0 ? subfamilies.map(sf => (
        ...
        >
          ...
          <div style={{ fontSize: 12, color: "#555", marginTop: 2 }}>
            ...
            {sfMeta[sf.name]?.extra && <span style={{ marginLeft: 6, color: "#444" }}>· {sfMeta[sf.name].extra}</span>}
          </div>
        </div>
      )) : (
        <>
          <p style={{ marginTop: 20 }}>
            <span style={{ color: SUBFAMILY_COLORS.Pantherinae }}>▊</span> Pantherinae
            <br /><span style={{ fontSize: 12 }}>2 genera · 7 species</span>
          </p>
          <p style={{ marginTop: 12 }}>
            <span style={{ color: SUBFAMILY_COLORS.Felinae }}>▊</span> Felinae
            <br /><span style={{ fontSize: 12 }}>12 genera · ~39 species · 7 lineages</span>
          </p>
        </>
      )}

      <div style={{ marginTop: 28, padding: "12px 14px", borderRadius: 8, background: "#111", fontSize: 12, lineHeight: 1.7 }}>
        <div style={{ color: "#888", marginBottom: 6 }}>Lineages in Felinae</div>
      ...
      <div style={{ marginTop: 28, fontSize: 11, color: "#444" }}>
        Data: Catalogue of Life ChecklistBank · Breeds: TICA/CFA
      </div>
    </div>
  );
}
...
export default function InfoPanel({ node, onSelect, findNodeById, subfamilies = [] }: Props) {
  if (!node || node.rank === "FAMILY") {
    return (
      <HomePanel subfamilies={subfamilies} onSelect={onSelect} />
    );
  }
    ...
    return (
      <div style={{ padding: "24px 20px", lineHeight: 1.6 }}>
        ...
        <div style={{ marginTop: 20, fontSize: 12, color: "#555", lineHeight: 1.6 }}>
          Interspecies hybrids exist only in captivity. Dashed lines in the tree connect each hybrid to its parent species.
        </div>
      </div>
    );
  }
  ...
  const childSubLabel = (child: TaxonNode) => {
    if (child.rank === "HYBRID_GROUP") {
      const n = (child.children ?? []).length;
      return `${n} hybrid${n === 1 ? "" : "s"}`;
    }
